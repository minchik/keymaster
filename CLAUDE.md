...
- `keymaster/keymaster/keymasterApp.swift` — the `@main` CLI surface only: the ArgumentParser command structs (`set`/`get`/`rm`/`run`/`version`) plus stdin/version/IO glue (`readSecret`, `fail`, `marketingVersion`). Each command builds a `SecretManager(backend: SystemKeychain())` and maps `KeychainError` → `fail(error.message)`.
- `keymaster/keymaster/SecretManager.swift` — **Foundation-only**, no `Security`/`LocalAuthentication`/`OSStatus` in any signature: the `KeychainError` enum, the `KeychainBackend`/`AuthSession` protocols, the testable `SecretManager` orchestration (upsert ordering, read-before-delete, the `run` batch flow), and the `decodeSecret`/`decodeEnvValue` UTF-8/NUL helpers. Compiled into the app **and** the test bundle.
- `keymaster/keymaster/SystemKeychain.swift` — app-target only: the real `KeychainBackend` adapter wrapping the `SecItem*`/`LAContext` syscalls, the access-control/access-group constants, and the `OSStatus` → `KeychainError` mapping. Unit-untestable (needs signing + Touch ID hardware), so it is covered by the manual checklist below.
...
The raw keychain/biometric/exec syscalls (`SystemKeychain.swift`) can't run headless or in CI — verify them by hand on a Touch ID Mac (below). The Foundation-only logic in `SecretManager.swift` and `RunSupport.swift` **is** unit-tested via the `keymasterTests` target (Swift Testing — `import Testing`, `@Test`/`#expect`). `SecretManager`'s ordering and decode invariants (upsert `add`→read→delete→add, read-before-delete on `remove`, the `run` batch flow, UTF-8/NUL rejection) are exercised against a `FakeKeychainBackend` (a dictionary-backed `KeychainBackend` that records call order and can throw programmed `KeychainError`s), so the security-critical logic is covered without any real Keychain access:
...
The `-only-testing:keymasterTests` scope is required: it excludes `keymasterUITests`, which launches the CLI app that `exit()`s immediately and would otherwise hang/fail the run. The `keymasterTests` bundle is **host-less** (`TEST_HOST`/`BUNDLE_LOADER` cleared) because the `@main` CLI exits before an app-hosted runner can attach; `RunSupport.swift` **and** `SecretManager.swift` are compiled into the bundle via synchronized-group membership exceptions (there is no `@testable import`), which is why both are kept Foundation-only with no `@main`/ArgumentParser/keychain (`Security`/`LocalAuthentication`) symbols. `SystemKeychain.swift` is deliberately **not** added to the membership exceptions — it holds the un-testable syscalls and stays out of the test bundle.
